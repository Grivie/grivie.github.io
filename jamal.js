  ...
  function ExposeBlocks(options)
  {
  	...
  	const init = () =>
  	{
  		const observer = new IntersectionObserver(scrollChecker,
  		{
  			threshold: 0
  		});
  	...
  	const addInterrupts = () =>
  	{
  		this.scrollers.forEach((scroller) =>
  		{
  			scroller.addEventListener("scroll", handleInterrupts,
  			{
  				passive: true
  			});
  		});
  	...
  	const removeInterrupts = () =>
  	{
  		this.scrollers.forEach((scroller) =>
  		{
  			scroller.removeEventListener("scroll", handleInterrupts,
  			{
  				passive: true
  			});
  		});
